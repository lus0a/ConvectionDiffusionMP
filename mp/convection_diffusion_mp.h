...
#ifndef __H__UG__LIB_DISC__CONVECTION_DIFFUSION__CONVECTION_DIFFUSION_MP__
#define __H__UG__LIB_DISC__CONVECTION_DIFFUSION__CONVECTION_DIFFUSION_MP__
...
class ConvectionDiffusionMP : public ConvectionDiffusionBase<TDomain>
{
	...
	///	Own type
		typedef ConvectionDiffusionMP<TDomain> this_type;
...
*/
	...
	///	computes the linearized defect w.r.t to the darcyW
		template <typename TElem, typename TFVGeom>
		void lin_def_darcyW(const LocalVector& u,
		                      std::vector<std::vector<MathVector<dim> > > vvvLinDef[],
		                      const size_t nip);
							  
	///	computes the linearized defect w.r.t to the darcyN
		template <typename TElem, typename TFVGeom>
		void lin_def_darcyN(const LocalVector& u,
		                      std::vector<std::vector<MathVector<dim> > > vvvLinDef[],
		                      const size_t nip);
							  
							  
/*							  
	///	computes zero linearized defect
		template <typename TElem, typename TFVGeom>
		void lin_def_saturationW(const LocalVector& u,
...
*/
	...
	///	computes the modified Sw
		template <typename TElem, typename TFVGeom>
		void ex_modified_value(number vValue[],
		              const MathVector<dim> vGlobIP[],
		              number time, int si,
		              const LocalVector& u,
		              GridObject* elem,
...
#endif /*__H__UG__LIB_DISC__CONVECTION_DIFFUSION__CONVECTION_DIFFUSION_MP__*/
